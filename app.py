...
from topic_modeling_draft_si_650 import get_topic_models, get_topic_models_lda
from extractive_summarizing_draft_si_650 import get_topic_models_graph, get_topic_models_random
...
@app.route("/twittername/<twitter_name>/top_tweets_random/<tweet_count>")
@cross_origin()  # allow all origins all methods.
def twitter_name_top_tweets_random(twitter_name, tweet_count):
    tw_handle = TwitterWrapper("")
    tw_handle.set_screen_name(twitter_name)
    ...
    results = get_topic_models_random(df.head(200), int(tweet_count))
    tweets = results['values']

    return jsonify(tweets)


@app.route("/twittername/<twitter_name>/top_tweets_lda/<tweet_count>")
@cross_origin()  # allow all origins all methods.
def twitter_name_top_tweets_lda(twitter_name, tweet_count):
    tw_handle = TwitterWrapper("")
    tw_handle.set_screen_name(twitter_name)
    ...
    df_complete = tw_handle.get_tweet_id_text(cache_only=cache_only)
    df = df_complete['text']

    results = get_topic_models_lda(df.head(200), df_complete, int(tweet_count))
    tweets = results['values']
